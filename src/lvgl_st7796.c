...
void lvgl_lcd_init(lv_disp_drv_t *drv)
{
    ...
    // Create SPI bus
    const spi_bus_config_t spi_bus_config = {.mosi_io_num = ST7796_SPI_MOSI, .miso_io_num = ST7796_SPI_MISO, .sclk_io_num = ST7796_SPI_SCLK, .quadwp_io_num = -1, .quadhd_io_num = -1};
    ...
    esp_lcd_panel_io_spi_config_t io_spi_config = {.cs_gpio_num = ST7796_CS, .dc_gpio_num = ST7796_DC, .spi_mode = SPI_MODE0, .pclk_hz = 24000000, .trans_queue_depth = 10, .lcd_cmd_bits = 8, .lcd_param_bits = 8};
    ...
    const esp_lcd_panel_dev_config_t panel_dev_config = {.reset_gpio_num = -1, .color_space = ESP_LCD_COLOR_SPACE_BGR, .bits_per_pixel = 16, .vendor_config = ST7796_VENDOR_CONFIG};
