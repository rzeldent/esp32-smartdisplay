...
void lvgl_tft_init(lv_disp_drv_t *drv)
{
    ...
    esp_lcd_panel_io_spi_config_t io_spi_config = LCD_IO_SPI_CONFIG;
    io_spi_config.on_color_trans_done = st7796_color_trans_done;
    io_spi_config.user_ctx = drv;
    esp_lcd_panel_io_handle_t io_handle;
    ESP_ERROR_CHECK(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)LCD_SPI_HOST, &io_spi_config, &io_handle));
