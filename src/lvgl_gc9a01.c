...
void lvgl_lcd_init(lv_disp_drv_t *drv)
{
    ...
    // Create SPI bus
    const spi_bus_config_t spi_bus_config = {.mosi_io_num = GC9A01_SPI_HOST, .sclk_io_num = GC9A01_SPI_SCLK, .quadwp_io_num = -1, .quadhd_io_num = -1};
    ...
    esp_lcd_panel_io_spi_config_t io_config = {.cs_gpio_num = GC9A01_CS, .dc_gpio_num = GC9A01_DC, .spi_mode = SPI_MODE0, .pclk_hz = 80000000, .trans_queue_depth = 10, .lcd_cmd_bits = 8, .lcd_param_bits = 8};
    ...
    esp_lcd_panel_dev_config_t panel_dev_config = {.reset_gpio_num = -1, .color_space = ESP_LCD_COLOR_SPACE_BGR, .bits_per_pixel = 16, .vendor_config = GC9A01_VENDOR_CONFIG};
