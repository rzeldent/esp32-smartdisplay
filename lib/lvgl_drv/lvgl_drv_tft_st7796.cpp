...
constexpr uint8_t CMD_FRMCTR2 = 0xB2;   // Frame Rate Control 2 (In Idle Mode/8 colors)
...
constexpr uint8_t CMD_PWR1 = 0xC0;      // Power Control 1
constexpr uint8_t CMD_PWR2 = 0xC1;      // Power Control 2
constexpr uint8_t CMD_PWR3 = 0xC2;      // Power Control 3
...
constexpr uint8_t MADCTL_RGB = 0x08; // RGB/BGR Order - 0=RGB, 1 = BGR
...
void lvgl_tft_st7796_send(const uint8_t command, const uint8_t data[] = nullptr, const ushort length = 0)
{
...
{
    lvgl_tft_st7796_send(CMD_SWRESET); // Software reset
    delay(100);
    lvgl_tft_st7796_send(CMD_SLPOUT);                         // Out of sleep mode
    static const uint8_t colmod[] = {0x55};                   // 16 bits R5G6B5
    lvgl_tft_st7796_send(CMD_COLMOD, colmod, sizeof(colmod)); // Set color mode
    static const uint8_t pwr1[] = {0x26};                     // AVDD=6.20v, AVCL=-4.8v
    lvgl_tft_st7796_send(CMD_PWR1, pwr1, sizeof(pwr1));       // Power control 1
    // VGH=?
    static const uint8_t pwr2[] = {0x11};                          // 4.4+(vcom+vcom offset)
    lvgl_tft_st7796_send(CMD_PWR2, pwr2, sizeof(pwr2));            // Power control 2
    static const uint8_t vcmpctrl[] = {0x35, 0x3E};                // 0x3E=53=1.625v
    lvgl_tft_st7796_send(CMD_VCMPCTL, vcmpctrl, sizeof(vcmpctrl)); // VCOM control
    //	{0xC7, 1,{0xBE}, 0},		 /*VCOM control*/
    lvgl_tft_st7796_send(CMD_INVOFF); // Inversion off
    lvgl_tft_st7796_send(CMD_NORON);  // Normal display on
    lvgl_tft_st7796_send(CMD_DISPON); // Main screen turn on
    delay(500);
};
...
void lvgl_tft_flush(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    lvgl_tft_st7796_send(CMD_CASET, caset, sizeof(caset));
    // Page addresses
    const uint8_t raset[] = {area->y1 >> 8, area->y1, area->y2 >> 8, area->y2};
    lvgl_tft_st7796_send(CMD_RASET, raset, sizeof(raset));
    // Memory write
    const auto size = lv_area_get_width(area) * lv_area_get_height(area);
    lvgl_tft_st7796_send(CMD_RAMWR, (uint8_t *)color_map, size * 2);
...
void lvgl_tft_set_orientation(enum lvgl_tft_orientation orientation)
{
    ...
    lvgl_tft_st7796_send(CMD_MADCTL, &data[orientation], 1);
}
...
void vgl_tft_sleep()
{
    ...
    lvgl_tft_st7796_send(CMD_SLPIN, slpin, sizeof(slpin));
}

void vgl_tft_wake()
{
    static const uint8_t splout[] = {0x08};
    lvgl_tft_st7796_send(CMD_SLPOUT, splout, sizeof(splout));
}
